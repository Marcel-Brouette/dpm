...
working_directory  = expanduser("~") + "/.marcelek_pass_manager/"
services_file_name = 'services.json'
global_data        = None
description        = '########## MARCELEK password manager ##########'
...
def hash(service_name) :
    ...
    if lvl == 1:
        revocationPattern = ' *' * version
        service_hash = b64encode(md5(globalPassword + " " + service_name + revocationPattern).digest())[:12]
    elif lvl == 3:
...
def first_use():
    print("It seems that you launch MPM&EK for the first time !")
    print("We need some of your inputs in order to use MPM&EK properly :")
    print("")
...
def set_level(service_name, level):
    if service_name not in services().keys():
        fatal_error("[ERROR] can't change security level on a service that doesn't exist.")
    else:
        services()[service_name][LVL] = level
        save_file()
        print("[SAVED] '%s' security level saved" % service_name)
        print_desc(service_name)
...
def print_desc(service_name):
    if service_name not in services().keys():
        ...
        lvl = services()[service_name].get(LVL, "")
...
def get_params():
    ...
    parser_arg.add_argument('-f', '--level', type=int, help='[can be used directly with --add] hardener the password output meeting cnil standard')
...
def run(args) :
    ...
    if 'infos' in args and not args.add: set_desc(args.service_name, args.infos)
    elif args.level and not args.add: set_level(args.service_name, args.level)
        ...
        initial_config = {
            ...
            SHARED  : args.shared,
            LVL    : args.level if args.level else 2
        }
